...
```{r}
mod_total <- lm(total_l ~ tail_l, possum)
mod_head <- lm(head_l ~ tail_l, possum)

summary(mod_total)$coefficients
summary(mod_total)$r.squared
```
...
```{r}
summary(lm(hwy ~ year, mpg))
```

__The intercept is the mean (or predicted value) of 1999 and the 2008 mean is the intercept plus $\beta_0$ (year2008 Estimate in the table)__
...
```{r}
summary(lm(hwy ~ displ + year, mpg))
```


2. Interpret your three coefficients, paying careful attention to units. 

$\beta_0$ is the predicted mpg of a car from 1999 with engine displ of $0$

$\beta_1$ is the rate at which mpg changes based on an increase of $1$ liter of displ while holding year fixed.

$\beta_2$ is the difference in mpg of a 2008 car versus a 1999 while holding displ fixed
